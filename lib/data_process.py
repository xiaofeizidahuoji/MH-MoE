...
def data_type_init(DATASET, args):
    if DATASET == 'PEMS07M' or DATASET == 'PEMS07L':
        data_type = 'speed'
    elif DATASET == 'PEMS08' or DATASET == 'PEMS04' or DATASET == 'CA_District5':
        data_type = 'flow'
...
def load_st_dataset(dataset, args):
    ...
    # 05/2012 - 06/2012 Tuesday
    elif dataset == 'PEMS07L':
...
def get_train_task_batch(args, x_list, y_list):
    dataset_use = [args.dataset_use]
    select_dataset = random.choice(dataset_use)
    print(dataset_use, select_dataset)
...
def get_val_tst_dataloader(X, Y, args, shuffle):
    X, Y = X[args.dataset_use], Y[args.dataset_use]
    data = torch.utils.data.TensorDataset(X, Y)
    data_loader = torch.utils.data.DataLoader(data, batch_size=args.batch_size, shuffle=shuffle, drop_last=False, num_workers=4)
    return data_loader
