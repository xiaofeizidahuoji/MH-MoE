...
def MAE_torch_all(pred, true, mask_value=None):
    if mask_value is not None:
        mask = torch.gt(true, mask_value)  
        pred[~mask] = 0  
        true[~mask] = 0 
    mae_loss = torch.abs(true - pred)  
    mae_loss_mean = torch.mean(mae_loss.squeeze(-1), dim=(-1, -2) , keepdim=False)
    return torch.mean(mae_loss_mean), mae_loss_mean, mae_loss
