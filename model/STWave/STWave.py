...
class temporalEmbedding(nn.Module):
    def __init__(self, D):
        ...
        self.ff_te = FeedForward([1447,D,D])

    def forward(self, TE, T=1440, W=7):
        '''
        TE:[B,T,2]
        '''
...
class STWave(nn.Module):
    ...
    """
    def __init__(self, args, args_base):
        ...
        adj_gat = args.adj_gat
        graphwave = args.gwv
        time_in_day_size, day_in_week_size, wave_type, wave_levels = \
            args.time_in_day_size, args.day_in_week_size, args.wave_type, args.wave_levels
    ...
    def forward(self, history_data, future_data, select_dataset=None, adap=None):
        '''
        x:[B,T,N,D]
        '''
        x = history_data
        te = torch.cat([x[:,:,0,-2:-1], x[:,:,0,-1:]], -1)
